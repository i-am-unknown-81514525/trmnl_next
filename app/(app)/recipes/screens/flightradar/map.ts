...
export function bboxForFlightDegrees(
	flight: FlightLocation,
	forwardRatio: number = 1.5,
	...
	const bLon =
		(backward * Math.sin(trackRad + Math.PI)) /
		Math.max(1e-4, Math.cos(latRad));
...
export function mergeBoxQueries<T>(
	boxes: BoundingBox[],
	query: (b: BoundingBox) => T[],
	idFn: (t: T) => string,
): T[] {
	const out: T[] = [];
...
function createEquirectangularProjector(
	bbox: BoundingBox,
	width: number,
	height: number,
	refLon: number,
...
function isFeature(x: unknown): x is Feature {
	return (
		!!x &&
		typeof x === "object" &&
		(x as any).type === "Feature" &&
		"geometry" in (x as any)
	);
}
...
export function drawMapBackground(
	...
	if (isFC(topoOrGeo)) {
		...
		const key =
			topoOrGeo.objects && (topoOrGeo.objects as any).land
				? "land"
				: Object.keys(topoOrGeo.objects)[0];
...
export function drawRotatedMapBackground(
	ctx: CanvasLike,
	topoOrGeo: TopoOrGeo,
	bbox: BoundingBox,
	width: number,
