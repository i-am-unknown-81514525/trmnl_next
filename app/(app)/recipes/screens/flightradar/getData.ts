...
import {
  FR24AirportDataArrivals,
  FR24AirportDataDepartures,
  FR24AirportListFlightResult,
  FR24AirportResult,
} from "./schema_external/fr24_airport_schedule";
...
async function getFlightInZone(
  ...
  for (let i = 1; i < entries; i++) {
    ...
    const squawk = view.getUint16(base + 32, true)
      ? view
          .getUint16(base + 32, true)
          .toString(16)
          .padStart(4, "0")
      : null;
...
async function getArrivalDepartureData(airport_code: string): Promise<{
  ...
  const arrival_response = await fetch(
    `https://api.flightradar24.com/common/v1/airport.json?code=${airport_code}&plugin[]=schedule&plugin-setting[schedule][mode]=arrivals&plugin-setting[schedule][timestamp]=${Math.floor(Date.now() / 1000)}&limit=100&page=1`,
  );
  const departure_response = await fetch(
    `https://api.flightradar24.com/common/v1/airport.json?code=${airport_code}&plugin[]=schedule&plugin-setting[schedule][mode]=departures&plugin-setting[schedule][timestamp]=${Math.floor(Date.now() / 1000)}&limit=100&page=1`,
  );
  if (!arrival_response.ok || !departure_response.ok) {
    throw new Error("Fail to collect arrival/departure data");
  }
