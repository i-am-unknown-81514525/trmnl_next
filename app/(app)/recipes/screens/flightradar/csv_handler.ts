...
export function loadAllSync(): void {
  ...
  const airport_rows: Record<string, string>[] = parse(airport_raw, {
    columns: true,
  ...
  if (airports.length > 0) {
    ...
    for (const a of airports) {
      ...
      if (nMax - nMin >= 0 && nMax - nMin <= 180) {
        ...
        const env1: [number, number, number, number] = [
          nMin,
          minLat,
          180,
          maxLat,
        ];
        const env2: [number, number, number, number] = [
          -180,
          minLat,
          nMax,
          maxLat,
  ...
  const runway_rows: Record<string, string>[] = parse(runway_raw, {
    columns: true,
    skip_empty_lines: true,
    trim: true,
  });
  for (const runway_row of runway_rows) {
    const airport_ident =
      (runway_row.airport_ident || "").trim().toUpperCase() || "";
  ...
  const navaid_rows: Record<string, string>[] = parse(navaid_raw, {
    columns: true,
    skip_empty_lines: true,
    trim: true,
  });
...
export function findRunwaysInBox(
  minLat: number,
  minLon: number,
  maxLat: number,
  maxLon: number,
...
export function findNavaidsInBox(
  minLat: number,
  minLon: number,
  maxLat: number,
  maxLon: number,
...
export function findAirportInBox(
  minLat: number,
  minLon: number,
  maxLat: number,
  maxLon: number,
